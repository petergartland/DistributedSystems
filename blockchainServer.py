...
message_list  =  []			
def sendMessageHelper(message, timestamp, clt_socket):
	t  =  threading.Thread(target=sendMessage, args=(message, timestamp, clt_socket))
	t.start()
	...
	#try:
		...
		if message.decode('utf-8')  ==  'time':
			...
			if not isValidMessage(client_socket, message.decode('utf-8')):
				return 'fuck'
			message_length = int(message_header.decode('utf-8'))
			return  {"header" : message_header, "data" : message, "time_header" : timestamp_header, "time" : timestamp.encode('utf-8')}	
		#except:
			return False
