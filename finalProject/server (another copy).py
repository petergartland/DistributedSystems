...
def addToBlockchain(message):
	global blockchain
	global pending_transactions #add stuff from this message to this data structure. then we need to add appropraite code in the removeFromBlockchain function then i think we are good! (might need to take care of, comment out, some del commands for this guy as well though).
